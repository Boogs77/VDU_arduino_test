//Mod by Boogs 2025-04-17

#define RW 3 //    Write Active LOW R/~W
#define RESET 9
...
void writeByte( unsigned char value) {         // Writes a byte to databus for register access
    ...
    PORTB = B00101111;                         // D13 D12 D11 D10 D09 D08 B X 1
    digitalWrite(RW, LOW);
...
unsigned char  readByte( ) {                   // Reads a byte from databus for register access
    ...
    digitalWrite(RW, HIGH);
    PORTB = B00101101;                         // D13 D12 D11 D10 D09 D08 B X 1
    delayMicroseconds(10);
    digitalWrite(RW, LOW);
...
void writeByteToVRAM( unsigned char value) {   //Writes a byte to databus for vram access Byte 3 for "WRITE TO VRAM" 
    ...
    digitalWrite(RW, LOW);
    PORTB = B00101110;                         // D13 D12 D11 D10 D09 D08 B X 0
    setDBWriteMode();                          // All outputs  
    setPort(value);                            // Write data
//    delayMicroseconds(10);
    digitalWrite(RW, HIGH);
...
unsigned char  readByteFromVRAM( ) {           //Reads a byte from databus for vram access Byte 3 for "READ FROM VRAM"
    ...
    digitalWrite(RW, HIGH);
    PORTB = B00101100;                         // D13 D12 D11 D10 D09 D08 B X 0       
    delayMicroseconds(1);
    memByte = readPort();                      // read data
    PORTB = 0x01;
    delayMicroseconds(10);    
...
void setup() {
  ...
  DDRB = DDRB | B11111111; // Set Pin 13 12 11 10 AND 8 as outputs. (A15 A14 A13 A12 xx xx xx A0)
  PORTB = 0x02;            // Disattiva indirizzo
  ...
  setRegister(7, 0xF5);   //Font & Background color (WHITE & LIGHT BLUE ) 
