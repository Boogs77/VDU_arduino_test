...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(MODE),call_int,FALLING); // ISR for clearing Interrupt flag 
  ...
  setRegister(1, 0xD0);   // Register 1: 1 1 0 1 0 0 0 0 ( 16K, Screen Enable, Interrupt Disable )
                          // Example table 6.1
  setRegister(2, 0x02);   //Name table              0X05 --> $1400
  setRegister(3, 0x00);   //Color table             0x80 --> $2000  
  ...
  unsigned int patternTableStart = 0x0000 * 0x800;
    ...
    // Name Table
    setWriteAddress(0x800 + 40*12 + 14);
...
void loop() { 
  //setRegister(7, 0xF0 | i);      //Testing the Backdrop Colors, defined by four bits in Register 7 i
  delay(1000);                   //The lower four bits contain the color for the Backdrop in all modes
  i++;                           // Font in LIghtblue, Background all 16 color
  if (i>15) i = 0;
